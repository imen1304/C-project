...
public:
  ...
  std::string ruler() const {  //style affichage  de la table en ligne, non pas le contenu
    std::string result;
    result += _corner;
  ...
  int correctDistance(const std::string& string_to_correct) const { //colonne est de meme largeur que colonne suivante
...
private:
  ...
  std::map<unsigned, Alignment> mutable _alignment; //mutable is used to allow a particular data of const object to be modified
  ...
  unsigned glyphLength(const std::string& s) const {
    ...
    std::setlocale(LC_ALL, "en_US.utf8"); //used only if width didn't work properly, so it's used by default
#else
  ...
  void determineWidths() const {
    ...
    for (auto rowIterator = _rows.begin(); rowIterator != _rows.end();
      ...
      for (unsigned i = 0; i < row.size(); ++i) {
        _width[i] =
            _width[i] > glyphLength(row[i]) ? _width[i] : glyphLength(row[i]); //width of each row  
      }
    }
  }

  void setupAlignment() const { //aligment of each column
    if (_rows.empty()) {
      return;
    }
...
inline std::ostream &operator<<(std::ostream &stream, const TextTable& table) { //verify that the stream is valid before printing to it
  if (table.rows().empty()) {
    return stream;
  }
  table.setup();
